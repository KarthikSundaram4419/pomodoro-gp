...
A race-inspired pomodoro flip-clock timer. One self-contained HTML file — no build step, no backend, no dependencies.
...
No accounts, no cookies, no analytics, no network requests. Preferences (durations, theme, sound) are stored in the visitor's own browser via `localStorage` and never leave it.
